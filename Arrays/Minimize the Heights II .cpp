...
Your Task:
You don't need to read input or print anything. Your task is to complete the function getMinDiff() which takes the arr[], 
...
class Solution {
  public:
    ...
    for(int i=0;i<n;i++)
    {
        if(arr[i]>=k)
        {
         m = min(arr[i]-k,arr[0]+k);
         l = max(arr[i-1]+k,arr[n-1]-k);
        d = min(d,l-m);
